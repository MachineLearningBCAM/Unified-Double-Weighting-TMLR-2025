function Experiments_DomainNet(idx1)

% Clear all

clearvars -except idx1 idx2
...
X = zscore([cell2mat(xTrain);xTest]);
...
nxTest = size(xTest, 1);
xTest = X(sum(nxTrain) + 1:sum(nxTrain) + nxTest, :);
...
for rep = 1:20
    ...
    for i = 1 : Mdl.distribs
        nxTrain(i) = size(xTrain{i,1},1);
        idx = randperm(nxTrain(i),min(n,nxTrain(i)));
        xTr{i,1} = xTrain{i,1}(idx,:);
        yTr{i,1} = yTrain{i,1}(idx);
    end
    nxTest = size(xTest,1);
    idx = randperm(nxTest,min(t,nxTest));
    xTe = xTest(idx,:);
    yTe = yTest(idx);
