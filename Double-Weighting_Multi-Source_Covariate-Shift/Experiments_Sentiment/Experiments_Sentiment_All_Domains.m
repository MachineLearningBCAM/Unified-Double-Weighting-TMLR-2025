...
for rep = 1:20
    ...
    yTe = [];
    for i = 1:4
        idx = randsample(size(Domains{i},1),1150);
        idx_tr = idx(151:1150);
        idx_te = idx(1:150);
        xTr{i,1} = Domains{i}(idx_tr,1:end-1);
        yTr{i,1} = Domains{i}(idx_tr,end);
        xTe = [xTe;Domains{i}(idx_te,1:end-1)];
        yTe = [yTe;Domains{i}(idx_te,end)];
